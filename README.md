...
npm run audit:full    # 5 audit: integrity, css-vars, ruby, content, related-ids
```
...
| Tests      | Vitest 4 + @testing-library/react (672)  |
...
Sebelum push, jalankan `npm run validate` — itu menambah `format:check` dan lima
skrip audit di `scripts/` yang tidak dijalankan CI.
