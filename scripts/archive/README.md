...
| `balance-option-lengths.mjs` | 2026-09-08 | Item 114, first pass. Trimmed a trailing parenthetical from 26 correct answers that were the longest option in their question — in 17 cases adopting the short form the *other* bank already shipped for the same question, and in every case leaving the detail in `exp`, plus the matching `opts_id`. Already applied; the data is committed. |
| `extract-jac-images.py` | 2026-09-08 | Produced the 12 files in `public/images/jac-official/` from the four JAC Official sample-question PDFs, one per photo question that had shipped with no photo. Kept for provenance: it records which page of which PDF each asset came from, and why each is a clip *render* of the page rather than an embedded-image extract — the blue arrow of `st1_q10` and the exam's own white label-masking are page drawings, so extracting the raster loses both. The source PDFs are JAC's and are not in this repository; the generated `.webp` files are the artefact. |
