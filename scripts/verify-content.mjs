...
// ── PART 5: card-index.js is a second generated artefact ────────────────────
// Added 2026-09-13 with the index itself. `cards.js` has been guarded here since a
// direct edit to it passed CI and was silently reverted at deploy; the index is
// generated by the same script from the same source and is now read by SRSContext
// and daily-mission, so it can go stale in exactly the same way — a card added to
// source/ but not merged would exist in the corpus and be invisible to the
// scheduler. Same generator, same gate.
log(`\n=== PART 5: card-index.js — ids and categories match cards.js ===\n`);
const indexPath = path.join(DATA_ROOT, 'card-index.js');
if (!fs.existsSync(indexPath)) {
  fail(`src/data/card-index.js is missing — run \`node scripts/merge-cards.mjs\``);
} else if (!cardsArr) {
  fail(`cards.js did not load, so the index cannot be compared against it`);
} else {
  const indexRes = loadDataModule(indexPath);
  if (!indexRes.ok) {
    fail(`card-index.js failed to parse: ${indexRes.error ?? indexRes.reason}`);
  } else {
    const { CARD_IDS, CARD_CATEGORY } = indexRes.exports;
    const problems = [];
    if (!Array.isArray(CARD_IDS)) {
      problems.push('CARD_IDS is not an array');
    } else {
      const realIds = cardsArr.map((c) => c.id);
      if (CARD_IDS.length !== realIds.length)
        problems.push(`CARD_IDS has ${CARD_IDS.length} ids, cards.js has ${realIds.length}`);
      for (let i = 0; i < Math.min(CARD_IDS.length, realIds.length); i++) {
        if (CARD_IDS[i] !== realIds[i]) {
          problems.push(`CARD_IDS[${i}] is ${CARD_IDS[i]}, cards.js has ${realIds[i]}`);
          break; // one is enough; the fix is the same
        }
      }
    }
    if (!CARD_CATEGORY || typeof CARD_CATEGORY !== 'object') {
      problems.push('CARD_CATEGORY is not an object');
    } else {
      for (const c of cardsArr) {
        if (CARD_CATEGORY[c.id] !== c.category) {
          problems.push(
            `card ${c.id}: index says category "${CARD_CATEGORY[c.id]}", cards.js says "${c.category}"`
          );
          break;
        }
      }
    ...
    if (problems.length === 0) {
      ok(`card-index.js matches cards.js (${CARD_IDS.length} ids, categories included)`);
    } else {
      for (const p of problems) fail(p);
      fail(`card-index.js is stale — run \`node scripts/merge-cards.mjs\``);
    }
  }
}
