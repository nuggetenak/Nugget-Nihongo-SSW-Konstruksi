# UI/UX Plan — 2026-08 overhaul (COMPLETED, ARCHIVED)

> **Historical. Every one of the 38 actionable items in this plan shipped.** Archived
> 2026-08-25 on completion. Superseded by `docs/UI_UX_PLAN.md` (a fresh plan, numbering
> items from 43 onward so references to "item 15" etc. in git history unambiguously mean
> *this* document).
>
> Provenance: written 2026-08-20 as the work queue for the `feat/ui-overhaul` branch,
> worked through across multiple sessions and completed 2026-08-25 at 546 passing tests.
> Kept in full because the per-item "How it fits" reasoning documents *why* choices were
> made, which the shipped code alone doesn't capture — several items also record routes
> deliberately **not** taken, and those are worth not re-litigating.
>
> Deferrals recorded here that were carried forward into the new plan: the `rem`
> conversion (§22), `correctFlash`/`wrongShake` visual parity across hand-rolled modes
> (§21), incremental mid-quiz persistence (§19), the in-app-exit history gap (§10), and
> `FilterPopup` wiring (§18).
