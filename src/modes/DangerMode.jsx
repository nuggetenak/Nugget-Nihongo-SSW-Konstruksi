...
export default function DangerMode({ onExit }) {
  ...
  const questions = useMemo(() => {
    ...
    return shuffle(DANGER_PAIRS).flatMap(pair => {
      ...
      return shuffle([0, 1]).slice(0, 1).map(askIdx => {
  ...
  if (!started) {
    return (
      <div style={{ padding: "24px 16px", maxWidth: T.maxW, margin: "0 auto" }}>
        ...
        <p style={{ fontSize: 13, color: T.textMuted, marginBottom: 12 }}>{DANGER_PAIRS.length} pasangan istilah mirip yang sering dijebak di ujian.</p>
