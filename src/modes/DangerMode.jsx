...
function PanelView({ onExit, onStartQuiz }) {
  ...
  return (
    <div style={{ padding: '24px 16px', maxWidth: T.maxW, margin: '0 auto' }}>
      <button onClick={onExit} style={{ fontFamily: 'inherit', fontSize: 12, color: T.textMuted, background: 'none', border: 'none', cursor: 'pointer', marginBottom: 16 }}>
        ← Kembali
      </button>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: T.textMuted }}>{DANGER_PAIRS.length} istilah yang sering salah di ujian</p>
        </div>
        <button
          onClick={onStartQuiz}
          style={{ fontFamily: 'inherit', padding: '9px 16px', fontSize: 13, fontWeight: 700, borderRadius: T.r.md, background: T.accent, border: 'none', color: T.textBright, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}
        >
          🧠 Drill
        </button>
      </div>
              ...
              {isOpen && (
                <div style={{ padding: '14px 16px', background: 'rgba(220,38,38,0.04)', border: `1px solid rgba(220,38,38,0.25)`, borderTop: 'none', borderRadius: `0 0 ${T.r.md}px ${T.r.md}px`, animation: 'fadeIn 0.15s ease' }}>
                  <div style={{ marginBottom: 10 }}>
                    ...
                    <div style={{ fontSize: 13, lineHeight: 1.65, color: T.text, padding: '8px 12px', background: T.correctBg, borderRadius: T.r.sm, border: `1px solid ${T.correctBorder}` }}>
                      {pair.correct}
                    </div>
                  </div>
                  <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                      {pair.traps.map((trap, ti) => (
                        <div key={ti} style={{ fontSize: 13, lineHeight: 1.65, color: T.textMuted, padding: '8px 12px', background: T.wrongBg, borderRadius: T.r.sm, border: `1px solid ${T.wrongBorder}` }}>
                          {trap}
                        </div>
                      ))}
                    </div>
    ...
    return (
      <div style={{ padding: '20px 16px', maxWidth: T.maxW, margin: '0 auto', animation: 'scaleIn 0.3s ease' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          <button onClick={restart} style={{ flex: 1, padding: '12px', fontSize: 13, fontWeight: 700, fontFamily: 'inherit', borderRadius: T.r.md, background: T.accent, border: 'none', color: T.textBright, cursor: 'pointer' }}>
            🔄 Ulang
          </button>
          <button onClick={onBack} style={{ flex: 1, padding: '12px', fontSize: 13, fontWeight: 700, fontFamily: 'inherit', borderRadius: T.r.md, background: T.surface, border: `1px solid ${T.border}`, color: T.textMuted, cursor: 'pointer' }}>
            📋 Panel
          </button>
        </div>
  ...
  return (
    <div style={{ padding: '12px 16px 24px', maxWidth: T.maxW, margin: '0 auto', animation: 'fadeIn 0.2s ease' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <button onClick={onBack} style={{ fontFamily: 'inherit', fontSize: 12, color: T.textMuted, background: 'none', border: 'none', cursor: 'pointer', padding: '6px 0' }}>
          ← Soal Jebak
        </button>
      ...
      {/* Question card */}
      <div style={{ padding: '20px 18px', background: T.surface, borderRadius: T.r.lg, border: `1px solid ${T.border}`, marginBottom: 14 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {opts.map((opt, i) => {
          ...
          const bg = !showResult ? T.surface
            : isCorrect ? T.correctBg
            : isWrongPick ? T.wrongBg
            : T.surface;
          const border = !showResult ? T.border
            : isCorrect ? T.correctBorder
            : isWrongPick ? T.wrongBorder
            : T.border;
          const color = !showResult ? T.text
            : isCorrect ? T.correct
            : isWrongPick ? T.wrong
            : T.textDim;
          return (
            <button
              ...
              style={{ fontFamily: 'inherit', padding: '12px 14px', borderRadius: T.r.md, background: bg, border: `1.5px solid ${border}`, color, textAlign: 'left', cursor: selected !== null ? 'default' : 'pointer', fontSize: 13, lineHeight: 1.5, transition: 'all 0.15s', display: 'flex', gap: 8, alignItems: 'flex-start' }}
            >
      ...
      {selected !== null && (
        <div style={{ marginTop: 14, padding: '14px 16px', borderRadius: T.r.md, background: T.surface, border: `1px solid ${T.border}`, animation: 'fadeIn 0.2s ease' }}>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, color: T.textDim, textTransform: 'uppercase', marginBottom: 10 }}>Penjelasan Perbedaan</div>
          <div style={{ marginBottom: 8 }}>
            <div style={{ fontSize: 10, color: T.correct, fontWeight: 700, marginBottom: 4 }}>✓ BENAR</div>
            <div style={{ fontSize: 12, lineHeight: 1.65, color: T.text, padding: '8px 12px', background: T.correctBg, borderRadius: T.r.sm, border: `1px solid ${T.correctBorder}` }}>
              {pair.correct}
            </div>
          </div>
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {pair.traps.map((trap, ti) => (
                <div key={ti} style={{ fontSize: 12, lineHeight: 1.65, color: T.textMuted, padding: '8px 12px', background: T.wrongBg, borderRadius: T.r.sm, border: `1px solid ${T.wrongBorder}` }}>
                  {trap}
                </div>
              ))}
            </div>
      ...
      {selected !== null && (
        <button
          ...
          style={{ width: '100%', marginTop: 12, padding: '13px', fontSize: 13, fontWeight: 700, fontFamily: 'inherit', borderRadius: T.r.md, border: 'none', background: T.accent, color: T.textBright, cursor: 'pointer', animation: 'fadeIn 0.15s ease' }}
        >
