  ...
  if (setKey === null) {
    return (
      <div style={{ padding: "24px 16px", maxWidth: T.maxW, margin: "0 auto" }}>
        ...
        <p style={{ fontSize: 13, color: T.textMuted, marginBottom: 20 }}>{JAC_OFFICIAL.length} soal dari contoh ujian resmi</p>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
          ...
          <button onClick={() => setShowID(f => !f)} style={{
            fontFamily: "inherit", fontSize: 11, padding: "6px 12px", borderRadius: T.r.pill, cursor: "pointer",
          ...
          }}>ID {showID ? "ON" : "OFF"}</button>
        </div>

        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {SETS.map(s => {
            const count = s.key === "all" ? JAC_OFFICIAL.length : JAC_OFFICIAL.filter(q => q.set === s.key).length;
            return (
              <button key={s.key} onClick={() => setSetKey(s.key)} style={{
