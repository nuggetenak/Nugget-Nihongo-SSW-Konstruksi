...
export default function SprintMode({ cards, onExit }) {
  ...
  if (phase === "ready") {
    return (
      <div style={{ padding: "24px 16px", maxWidth: T.maxW, margin: "0 auto", textAlign: "center" }}>
        <button onClick={onExit} style={{ fontFamily: "inherit", fontSize: 12, color: T.textMuted, background: "none", border: "none", cursor: "pointer", marginBottom: 24, display: "block" }}>← Kembali</button>
        <div style={{ fontSize: 48, marginBottom: 16 }}>⚡</div>
        <h2 style={{ fontSize: 20, fontWeight: 800, marginBottom: 8 }}>Sprint Mode</h2>
        <p style={{ fontSize: 13, color: T.textMuted, marginBottom: 24 }}>Jawab sebanyak-banyaknya dalam {DURATION} detik!</p>
    ...
    return (
      <div style={{ padding: "24px 16px", maxWidth: T.maxW, margin: "0 auto", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => { setPhase("ready"); setIdx(0); setCorrect(0); setWrong(0); setTimeLeft(DURATION); setOrder(shuffle(cards)); }} style={{
  ...
  return (
    <div style={{ padding: "16px 16px 24px", maxWidth: T.maxW, margin: "0 auto" }}>
      ...
      <div style={{
        ...
        border: `1px solid ${T.border}`, minHeight: 180,
        display: "flex", flexDirection: "column", justifyContent: "center",
      }}>
        <JpFront jp={card.jp} furi={card.furi} romaji={card.romaji} />
        {showAnswer && (
          <div style={{ textAlign: "center", marginTop: 12, fontSize: 14, color: T.gold, fontWeight: 600 }}>{card.id_text}</div>
        )}
      </div>
      ...
      {!showAnswer && (
        <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
          <button onClick={handleDontKnow} style={{
            ...
            background: T.wrongBg, border: `1px solid ${T.wrongBorder}`, color: T.wrong,
          }}>Tidak tahu</button>
          <button onClick={handleKnow} style={{
            flex: 1, padding: "14px", fontSize: 14, fontWeight: 600,
            fontFamily: "inherit", borderRadius: T.r.md, cursor: "pointer",
            background: T.correctBg, border: `1px solid ${T.correctBorder}`, color: T.correct,
          }}>Tahu! ✓</button>
        </div>
      )}
    </div>
