...
export default function ConfusionMode({ onExit, onSessionEnd }) {
  ...
  const filtered = filterType === 'all'
    ? CONFUSION_PAIRS
    : CONFUSION_PAIRS.filter((p) => p.type === filterType);

  if (view === 'quiz') {
    return <QuizView pairs={filtered} onBack={() => setView('panel')} onSessionEnd={onSessionEnd} />;
  }
  ...
  return (
    <PanelView
      ...
      onOpenDetail={(pair) => { setSelectedPair(pair); setView('detail'); }}
    />
  );
}

// ── Panel ─────────────────────────────────────────────────────────────────────
function PanelView({ pairs, filtered, filterType, onFilterChange, onExit, onStartQuiz, onOpenDetail }) {
  ...
  return (
    <div className={S.page}>
      ...
      <div className={S.rowSpread} style={{ marginBottom: 4, alignItems: 'flex-start' }}>
        ...
        <button className={S.btnPrimary} style={{ padding: '10px 18px', fontSize: 13 }} onClick={onStartQuiz}>
          🧠 Kuis
        </button>
      </div>
        ...
        {types.map((t) => {
          ...
          return (
            <button
              ...
              style={{
                fontFamily: 'inherit', padding: '5px 12px', fontSize: 12, borderRadius: T.r.pill,
            ...
            >
              {t === 'all' ? `Semua (${typeCount(t)})` : `${meta.label} · ${meta.desc} (${typeCount(t)})`}
            </button>
          );
        })}
      </div>
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <span style={{
                    fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: T.r.pill,
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 16, fontWeight: 700, fontFamily: 'Noto Sans JP, sans-serif', color: T.text }}>
                    {pair.termA}
                  </div>
...
function DetailView({ pair, onBack }) {
  ...
  return (
    <div className={S.pageFade} style={{ padding: 'var(--sp-5) var(--sp-4)' }}>
      <button className={S.btnBack} onClick={onBack}>← Kata Mirip</button>

      <div style={{ marginBottom: 4 }}>
        <span style={{
          fontSize: 11, fontWeight: 700, padding: '3px 9px', borderRadius: T.r.pill,
      ...
      <div className={S.card} style={{ marginBottom: 12, borderLeft: `3px solid ${T.amber}` }}>
        <div style={{ fontSize: 24, fontWeight: 800, fontFamily: 'Noto Sans JP, sans-serif', marginBottom: 4 }}>
          {pair.termA}
        </div>
        {pair.furiA && <div style={{ fontSize: 12, color: T.textDim, marginBottom: 8 }}>{pair.furiA}</div>}
        <div style={{ fontSize: 14, color: T.text, lineHeight: 1.6 }}>{pair.defA}</div>
      </div>

      {/* Term B */}
      <div className={S.card} style={{ marginBottom: 12, borderLeft: `3px solid ${T.borderActive}` }}>
        <div style={{ fontSize: 24, fontWeight: 800, fontFamily: 'Noto Sans JP, sans-serif', marginBottom: 4 }}>
          {pair.termB}
        </div>
        {pair.furiB && <div style={{ fontSize: 12, color: T.textDim, marginBottom: 8 }}>{pair.furiB}</div>}
        <div style={{ fontSize: 14, color: T.text, lineHeight: 1.6 }}>{pair.defB}</div>
      </div>

      {/* Tip */}
      {pair.tip && (
        <div className={S.card} style={{ background: `rgba(245,158,11,0.08)`, border: `1px solid rgba(245,158,11,0.25)` }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: T.amber, marginBottom: 6 }}>💡 Cara Bedakan</div>
    ...
    return (
      <div className={S.pageScroll} style={{ padding: 'var(--sp-5) var(--sp-4)' }}>
        <div style={{
          ...
          marginBottom: 20, animation: 'popIn 0.35s var(--ease-spring) both',
        }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          <button className={S.btnPrimary} style={{ flex: 1 }} onClick={() => {
            setQIdx(0); setSelected(null); setResults([]); setPhase('playing'); sessionFired.current = false;
          }}>🔄 Ulang</button>
          <button className={S.btnSecondary} style={{ flex: 1 }} onClick={onBack}>📋 Panel</button>
        </div>
              ...
              {wrongList.map((r, i) => {
                ...
                return (
                  <div key={i} className={S.card} style={{
                    ...
                    <div style={{ display: 'flex', gap: 8, marginBottom: 8, fontFamily: 'Noto Sans JP, sans-serif' }}>
  ...
  return (
    <div className={S.pageScroll} style={{ padding: 'var(--sp-4)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <button className={S.btnBack} style={{ marginBottom: 0 }} onClick={onBack}>← Kata Mirip</button>
      ...
      <div style={{ textAlign: 'center', margin: '12px 0 8px' }}>
        <span style={{
          fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: T.r.pill,
          background: meta.bg, color: meta.color,
        }}>
          {meta.desc}
        </span>
      ...
      <div style={{
        ...
        padding: '16px 14px', marginBottom: 16, animation: 'scaleIn 0.2s var(--ease-smooth)',
      }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 20, fontWeight: 800, fontFamily: 'Noto Sans JP, sans-serif', color: T.amber }}>
            {pair.termA}
          </div>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 20, fontWeight: 800, fontFamily: 'Noto Sans JP, sans-serif', color: T.text }}>
            {pair.termB}
          </div>
      ...
      {/* Question prompt */}
      <div style={{
        fontSize: 13, color: T.textMuted, textAlign: 'center', marginBottom: 12, fontWeight: 600,
      }}>
        ❓ Pilih arti yang benar untuk <span style={{ color: T.amber, fontFamily: 'Noto Sans JP, sans-serif' }}>{pair.termA}</span>
      </div>
        ...
        {opts.map((opt, i) => {
          ...
          return (
            <button
              ...
              disabled={showResult}
              style={{
                fontFamily: 'inherit', padding: '14px 16px', borderRadius: 12,
                background: !showResult ? T.surface : isCorrectOpt ? T.correctBg : isWrongPick ? T.wrongBg : T.surface,
                border: `1.5px solid ${!showResult ? T.border : isCorrectOpt ? T.correctBorder : isWrongPick ? T.wrongBorder : T.border}`,
                color: !showResult ? T.text : isCorrectOpt ? T.correct : isWrongPick ? T.wrong : T.textDim,
                textAlign: 'left', cursor: showResult ? 'default' : 'pointer',
                fontSize: 13, lineHeight: 1.5, transition: 'all var(--t-fast)',
      ...
      {showResult && (
        <div className={S.card} style={{
          ...
          {pair.tip && (
            <div style={{ fontSize: 12, color: T.text, lineHeight: 1.5, borderTop: `1px solid rgba(245,158,11,0.15)`, paddingTop: 8 }}>
              💡 {pair.tip}
            </div>
          )}
        </div>
