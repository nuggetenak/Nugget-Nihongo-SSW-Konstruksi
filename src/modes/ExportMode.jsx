...
export default function ExportMode({ onExit }) {
  ...
  const handleExport = () => {
    ...
    try {
      ...
      setStatus({
        type: 'ok',
        msg: `✅ Berhasil! ${Object.keys(progressData).length} kunci progress + ${srsCount} kartu SRS disimpan.`,
      });
    } catch (e) {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      setStatus({
        type: 'ok',
        msg: `✅ Berhasil! ${progressCount} kunci + ${srsCount} kartu SRS dipulihkan. Reload untuk melihat perubahan.`,
      });
    ...
    <div style={{ padding: '24px 16px', maxWidth: T.maxW, margin: '0 auto' }}>
      ...
      >
        ...
        <div style={{ marginTop: 10, fontSize: 11, color: T.textDim, lineHeight: 1.6 }}>
          ⚠️ Progress tersimpan di browser ini saja. Gunakan ekspor/impor untuk pindah ke perangkat
          lain.
        </div>
      </div>
    </div>
