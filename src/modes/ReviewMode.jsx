...
export default function ReviewMode({ srs, onExit }) {
  ...
  const [lastResult, setLast]   = useState(null);  // { interval, rating }
  ...
  if (done) {
    ...
    return (
      <div style={{ padding: '40px 20px', textAlign: 'center', maxWidth: T.maxW, margin: '0 auto', animation: 'fadeIn 0.3s ease' }}>
        <div style={{ fontSize: 52, marginBottom: 12 }}>{total === 0 ? '🎉' : pct >= 70 ? '🏆' : '📚'}</div>
        ...
        <button onClick={onExit} style={{
          ...
          fontFamily: 'inherit', borderRadius: T.r.md, border: 'none', cursor: 'pointer',
  ...
  return (
    <div style={{ padding: '12px 16px 32px', maxWidth: T.maxW, margin: '0 auto', animation: 'fadeIn 0.2s ease' }}>
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <button onClick={onExit} style={{ fontFamily: 'inherit', fontSize: 12, color: T.textMuted, background: 'none', border: 'none', cursor: 'pointer', padding: '6px 0' }}>← Keluar</button>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginTop: 10, marginBottom: 2 }}>
        <span style={{
          fontSize: 10, padding: '2px 8px', borderRadius: T.r.pill,
          background: `${info.strength.color}15`, color: info.strength.color,
      ...
      <div
        ...
        style={{
          marginTop: 10, padding: flipped ? '22px 18px' : '36px 20px',
        ...
        {flipped && (
          <div style={{ animation: 'fadeIn 0.15s ease', borderTop: `1px solid ${T.border}`, paddingTop: 14 }}>
            ...
            <div style={{ marginTop: 10, textAlign: 'center' }}>
              <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: T.r.pill, background: `${cat.color}15`, color: cat.color, border: `1px solid ${cat.color}33` }}>
                {cat.emoji} {cat.label}
              </span>
            </div>
          </div>
      ...
      {!flipped ? (
        ...
        <div style={{ marginTop: 14 }}>
          <div style={{ fontSize: 10, color: T.textDim, textAlign: 'center', marginBottom: 8, letterSpacing: 0.5 }}>
