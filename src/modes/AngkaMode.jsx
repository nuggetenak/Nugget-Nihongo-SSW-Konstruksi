    ...
    <div style={{ padding: '24px 16px', maxWidth: T.maxW, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: T.textMuted }}>{ANGKA_KUNCI.length} angka WAJIB hafal sebelum ujian</p>
        </div>
        <button
          onClick={onStartQuiz}
              ...
              return (
                <div key={key}>
                  <button
                    ...
                    style={{ fontFamily: 'inherit', width: '100%', padding: '11px 14px', borderRadius: isOpen ? `${T.r.md}px ${T.r.md}px 0 0` : T.r.md, background: isOpen ? `${g.color}14` : T.surface, border: `1px solid ${isOpen ? `${g.color}40` : T.border}`, color: T.text, textAlign: 'left', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12 }}
                  >
                    <span style={{ fontSize: 14, fontWeight: 800, color: g.color, fontVariantNumeric: 'tabular-nums', flexShrink: 0, minWidth: 64 }}>
                      {item.angka}
                    </span>
                    <span style={{ fontSize: 12, color: T.textMuted, flex: 1, lineHeight: 1.4, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                      {item.konteks}
                    </span>
...
function QuizView({ onBack }) {
              ...
              {wrongList.map((r, i) => {
                ...
                return (
                  <div key={i} style={{ padding: '12px 14px', borderRadius: T.r.md, background: T.surface, borderLeft: `3px solid ${T.wrong}`, animation: `slideUp 0.3s ease ${i * 0.05}s both` }}>
                    ...
                    <div style={{ fontSize: 12, color: T.wrong, marginBottom: 3 }}>✗ {pickedText}</div>
  ...
  return (
    <div style={{ padding: '12px 16px 24px', maxWidth: T.maxW, margin: '0 auto', animation: 'fadeIn 0.2s ease' }}>
      ...
      <ProgressBar current={qIdx + (selected !== null ? 1 : 0)} total={items.length} color={color} />
      <div style={{ fontSize: 11, color: T.textFaint, marginTop: 8, marginBottom: 14, fontVariantNumeric: 'tabular-nums' }}>
        {qIdx + 1} / {items.length}
      </div>

      {/* Question card */}
      <div style={{ padding: '20px 18px', background: `${color}0E`, borderRadius: T.r.lg, border: `2px solid ${color}35`, marginBottom: 14, animation: 'fadeIn 0.2s ease' }}>
        <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 2, color, textTransform: 'uppercase', marginBottom: 10 }}>Angka berapa untuk…</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {opts.map((opt, i) => {
          ...
          const bg = !showResult ? T.surface : isCorrect ? T.correctBg : isWrongPick ? T.wrongBg : T.surface;
          const border = !showResult ? T.border : isCorrect ? T.correctBorder : isWrongPick ? T.wrongBorder : T.border;
          const txtColor = !showResult ? T.text : isCorrect ? T.correct : isWrongPick ? T.wrong : T.textDim;
          return (
            <button
              key={i}
              onClick={() => handleSelect(i)}
              disabled={selected !== null}
              style={{ fontFamily: 'inherit', padding: '12px 14px', borderRadius: T.r.md, background: bg, border: `1.5px solid ${border}`, color: txtColor, textAlign: 'left', cursor: selected !== null ? 'default' : 'pointer', fontSize: 14, fontWeight: 700, fontVariantNumeric: 'tabular-nums', transition: 'all 0.15s', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
            >
              <span>{opt.text}</span>
