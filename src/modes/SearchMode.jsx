    ...
    <div style={{ padding: "16px 16px 24px", maxWidth: T.maxW, margin: "0 auto" }}>
      ...
      <input
        ...
        style={{
          ...
          color: T.text, outline: "none", marginBottom: 16,
        }}
      />
          ...
          return (
            <div key={c.id} style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ fontFamily: T.fontJP, fontSize: 15, fontWeight: 600 }}>{stripFuri(c.jp)}</div>
                <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: T.r.pill, background: `${cat.color}22`, color: cat.color, whiteSpace: "nowrap" }}>{cat.emoji}</span>
              </div>
              ...
              {c.desc && <div style={{ fontSize: 11, color: T.textMuted, marginTop: 4, lineHeight: 1.5 }}>{c.desc.slice(0, 100)}{c.desc.length > 100 ? "…" : ""}</div>}
            </div>
          );
        })}
      </div>
