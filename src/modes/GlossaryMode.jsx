...
export default function GlossaryMode({ onExit }) {
  ...
  const sorted = useMemo(() => {
    let items = filterCat === "all" ? CARDS : CARDS.filter(c => c.category === filterCat);
  ...
  return (
    <div style={{ padding: "16px 16px 24px", maxWidth: T.maxW, margin: "0 auto" }}>
      <button onClick={onExit} style={{ fontFamily: "inherit", fontSize: 12, color: T.textMuted, background: "none", border: "none", cursor: "pointer", marginBottom: 12 }}>← Kembali</button>
      <h2 style={{ fontSize: 18, fontWeight: 800, marginBottom: 4 }}>📖 Glosari</h2>
      <p style={{ fontSize: 12, color: T.textDim, marginBottom: 12 }}>{sorted.length} istilah terurut</p>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 16 }}>
        {[{ key: "all", label: "Semua", emoji: "📋" }, ...CATEGORIES.filter(c => c.key !== "all" && c.key !== "bintang")].map(c => (
          <button key={c.key} onClick={() => setFilterCat(c.key)} style={{
            fontFamily: "inherit", fontSize: 10, padding: "4px 10px", borderRadius: T.r.pill, cursor: "pointer",
      ...
      {groups.map(([letter, items]) => (
        <div key={letter}>
          <div style={{ fontSize: 13, fontWeight: 800, color: T.gold, padding: "8px 0 4px", borderBottom: `1px solid ${T.border}`, marginBottom: 4 }}>{letter}</div>
          {items.map(c => {
              ...
              <div key={c.id} onClick={() => setExpanded(isOpen ? null : c.id)} style={{
                ...
                {isOpen && (
                  <div style={{ marginTop: 6, fontSize: 11, color: T.textMuted, lineHeight: 1.5 }}>
                    {c.furi && <div style={{ color: T.textDim, fontFamily: T.fontJP, marginBottom: 2 }}>{c.furi} · {c.romaji}</div>}
                    {c.desc}
                  </div>
                )}
              </div>
