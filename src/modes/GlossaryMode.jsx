  ...
  return (
    <div style={{ paddingBottom: 88 }}>
      <div style={{ padding: '16px 16px 10px', maxWidth: T.maxW, margin: '0 auto' }}>
        ...
        <div className={S.row} style={{ alignItems: 'baseline', gap: 8, marginBottom: 2 }}>
          <h2 style={{ fontSize: 18, fontWeight: 800, margin: 0 }}>📖 Glosari</h2>
          <span className={S.pill} style={{ fontSize: 11, color: T.textDim, background: T.surface, border: `1px solid ${T.border}` }}>
      ...
      {/* Sticky A-Z nav */}
      <div ref={navRef} style={{ position: 'sticky', top: 0, zIndex: 20, overflowX: 'auto', display: 'flex', gap: 2, padding: '5px 12px', background: T.bg, borderTop: `1px solid ${T.border}`, borderBottom: `1px solid ${T.border}`, scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' }}>
        {letters.map((letter) => {
          const isActive = activeLetter === letter;
          return (
            <button key={letter} data-nav={letter} onClick={() => jumpTo(letter)} style={{ flexShrink: 0, fontFamily: T.fontJP, fontSize: 12, fontWeight: isActive ? 800 : 500, minWidth: 28, height: 28, borderRadius: T.r.sm, cursor: 'pointer', border: isActive ? '1px solid rgba(251,191,36,0.45)' : '1px solid transparent', background: isActive ? 'rgba(251,191,36,0.18)' : 'transparent', color: isActive ? T.gold : T.textDim, transition: 'all 0.15s', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {letter}
            </button>
          );
        })}
      ...
      <div style={{ maxWidth: T.maxW, margin: '0 auto', padding: '0 0 8px' }}>
        {groups.map(([letter, items]) => (
          <div key={letter} data-letter={letter} ref={(el) => { sectionRefs.current[letter] = el; }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 16px 5px', borderBottom: `1px solid ${T.borderLight}`, marginTop: 2 }}>
              <span style={{ fontFamily: T.fontJP, fontSize: 15, fontWeight: 900, color: T.gold, letterSpacing: '0.02em' }}>{letter}</span>
              <span style={{ fontSize: 10, color: T.textFaint, background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.r.pill, padding: '1px 7px' }}>{items.length}</span>
            </div>
            {items.map((c) => {
              const isOpen = expanded === c.id;
              const catInfo = catMap[c.category];
              return (
                <div key={c.id} onClick={() => setExpanded(isOpen ? null : c.id)} style={{ padding: '9px 16px', cursor: 'pointer', borderBottom: `1px solid ${T.border}`, background: isOpen ? T.surface : 'transparent', transition: 'background 0.12s' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
                      {catInfo && <span style={{ fontSize: 11, flexShrink: 0, opacity: 0.6 }}>{catInfo.emoji}</span>}
                      <span style={{ fontFamily: T.fontJP, fontSize: 14, fontWeight: 600, color: T.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{stripFuri(c.jp)}</span>
                    </div>
                    <span style={{ fontSize: 11, color: T.textMuted, flexShrink: 0, textAlign: 'right', maxWidth: '45%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.id_text}</span>
                  </div>
                  {isOpen && (
                    <div style={{ marginTop: 8, paddingTop: 8, borderTop: `1px solid ${T.border}`, animation: 'fadeIn 0.15s ease' }}>
                      ...
                      {c.source && (
                        <div style={{ marginTop: 6 }}>
                          <span style={{ fontSize: 9, color: T.textFaint, background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.r.pill, padding: '1px 6px' }}>{c.source}</span>
                        </div>
                      )}
                    </div>
                  )}
