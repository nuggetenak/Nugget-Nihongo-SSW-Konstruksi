...
function NoteCard({ card, note, onSave }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 12, border: `1px solid ${note ? 'var(--ssw-borderLight)' : 'var(--ssw-border)'}`,
      marginBottom: 10,
    }}>
      <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
        ...
        <button
          onClick={() => { setDraft(note || ''); setEditing(!editing); }}
          style={{
            background: 'none', border: 'none', cursor: 'pointer',
            fontSize: 18, padding: '4px', color: note ? 'var(--ssw-amber)' : 'var(--ssw-textFaint)',
          }}
      ...
      {note && !editing && (
        <div style={{
          ...
          fontSize: 13, color: 'var(--ssw-text)', lineHeight: 1.5,
        ...
        <div style={{ marginTop: 10 }}>
          <textarea
            ...
            style={{
              ...
              background: 'var(--ssw-inputBg)', border: '2px solid var(--ssw-borderLight)',
              borderRadius: 8, fontSize: 13, color: 'var(--ssw-text)',
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
            <button
              ...
              style={{
                ...
                background: 'var(--ssw-amber)', color: '#fff',
            ...
            {note && (
              <button
                ...
                style={{
                  ...
                  background: 'var(--ssw-wrongBg)', color: 'var(--ssw-wrong)',
                  fontFamily: 'inherit', fontSize: 13, fontWeight: 600,
                  border: '1px solid var(--ssw-wrongBorder)', cursor: 'pointer',
                }}
              >
                Hapus
              </button>
            ...
            <button
              ...
              style={{
                padding: '9px 16px', borderRadius: 8,
                background: 'var(--ssw-surface)', color: 'var(--ssw-textMuted)',
...
export default function CatatanMode({ cards, onExit }) {
  ...
  const handleSave = useCallback((cardId, text) => {
  ...
  const filtered = useMemo(() => {
    ...
    if (debouncedQuery) {
      ...
      pool = pool.filter(
        (c) => stripFuri(c.jp).toLowerCase().includes(q) ||
  ...
  const noteCount = Object.keys(notes).filter((id) =>
  ...
  return (
    <div className={S.pageScroll}>
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--ssw-textBright)', marginBottom: 2 }}>
          📓 Buku Catatan
        </div>
        <div style={{ fontSize: 13, color: 'var(--ssw-textMuted)' }}>
      ...
      <input
        ...
        style={{
          ...
          background: 'var(--ssw-inputBg)', border: '2px solid var(--ssw-border)',
          borderRadius: 10, fontSize: 14, color: 'var(--ssw-text)',
      ...
      {/* Card list */}
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--ssw-textFaint)', fontSize: 14 }}>
        ...
        <>
          ...
          {filtered.length < (filter === 'semua' ? cards.length : filtered.length) && (
            <div style={{ textAlign: 'center', fontSize: 12, color: 'var(--ssw-textFaint)', paddingTop: 8 }}>
