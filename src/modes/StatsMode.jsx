...
export default function StatsMode({ known, unknown, quizWrong = {}, onExit }) {
  ...
  // Category breakdown
  const catStats = CATEGORIES.filter(c => c.key !== "all" && c.key !== "bintang").map(cat => {
  ...
  return (
    <div style={{ padding: "24px 16px", maxWidth: T.maxW, margin: "0 auto" }}>
      ...
      {/* Overview Card */}
      <div style={{ padding: "20px 16px", background: T.surface, borderRadius: T.r.lg, border: `1px solid ${T.border}`, marginBottom: 20, textAlign: "center" }}>
      ...
      {/* Category Breakdown */}
      <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.5, color: T.textDim, textTransform: "uppercase", marginBottom: 8 }}>Per Kategori</div>
      ...
      {weakest.length > 0 && (
        <>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.5, color: T.textDim, textTransform: "uppercase", marginBottom: 8 }}>⚠️ Perlu Fokus</div>
      ...
      {wrongEntries.length > 0 && (
        <>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.5, color: T.textDim, textTransform: "uppercase", marginBottom: 8 }}>Sering Salah</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {wrongEntries.map(e => {
              const card = CARDS.find(c => c.id === e.id);
              if (!card) return null;
              return (
                <div key={e.id} style={{ display: "flex", justifyContent: "space-between", padding: "8px 12px", background: T.surface, borderRadius: T.r.sm, fontSize: 12 }}>
