...
export default function FlashcardMode({ cards, known, unknown, onMark, onExit, srs }) {
  ...
  useEffect(() => {
    ...
    setIdx(0); setFlipped(false); setRated(false); setLastRating(null);
  ...
  const go = useCallback((dir) => {
    ...
    setTimeout(() => {
      ...
      setFlipped(false); setSwipeDir(0); setRated(false); setLastRating(null);
    }, 120);
  }, [order.length]);
    ...
    <div style={{ padding: '12px 16px 24px', maxWidth: T.maxW, margin: '0 auto', animation: 'fadeIn 0.2s ease' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
          {info && (
            <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: T.r.pill, background: `${info.strength.color}15`, color: info.strength.color }}>
              {info.strength.label}
            </span>
          )}
      ...
      <div
        ...
        style={{
          marginTop: 14, padding: flipped ? '22px 18px' : '32px 20px',
          background: T.surface, borderRadius: T.r.xxl,
          border: `1.5px solid ${borderColor}`,
          minHeight: 220, cursor: flipped ? 'default' : 'pointer',
      ...
      >
        ...
        <div style={{ textAlign: 'center', marginBottom: flipped ? 14 : 0 }}>
          ...
          {card.furi && <div style={{ fontSize: 12, color: T.textMuted, fontFamily: T.fontJP, marginTop: 4 }}>{card.furi}</div>}
          <div style={{ fontSize: 11, color: T.textDim, fontStyle: 'italic', marginTop: 2 }}>{card.romaji}</div>
        </div>

        {/* Back */}
        {flipped && (
          <div style={{ animation: 'fadeIn 0.15s ease', borderTop: `1px solid ${T.border}`, paddingTop: 12 }}>
            <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 8, textAlign: 'center', color: T.gold }}>{card.id_text}</div>
            <div style={{ fontSize: 12, lineHeight: 1.7, color: T.textMuted }}>{card.desc}</div>
            <div style={{ marginTop: 10, textAlign: 'center' }}>
              <span style={{ fontSize: 10, padding: '3px 10px', borderRadius: T.r.pill, background: `${cat.color}15`, color: cat.color, border: `1px solid ${cat.color}33` }}>
                {cat.emoji} {cat.label}
              </span>
            </div>
          </div>
      ...
      <div style={{ textAlign: 'center', fontSize: 10, color: T.textFaint, marginTop: 8, letterSpacing: 0.5 }}>
        {flipped
          ? rated ? '' : 'Nilai pemahamanmu · 1/2/3/4'
          ...
          {[1, 2, 3, 4].map(rating => {
            ...
            return (
              <button
                ...
                style={{
                  fontFamily: 'inherit', padding: '10px 4px', borderRadius: T.r.md, cursor: 'pointer',
      ...
      {rated && lastRating && (
        <div style={{
          marginTop: 12, padding: '10px', borderRadius: T.r.md, textAlign: 'center',
          background: RATING_META[lastRating].bg, border: `1px solid ${RATING_META[lastRating].border}`,
          color: RATING_META[lastRating].color, fontSize: 13, fontWeight: 700,
