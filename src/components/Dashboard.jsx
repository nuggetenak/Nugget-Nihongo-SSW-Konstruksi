...
export default function Dashboard({ known, unknown, track, filteredCount, onNavigate, onChangeTrack, srs }) {
  ...
  const getSuggestion = () => {
    if (dueCount > 0)
      return { icon: '🔁', label: `${dueCount} kartu siap diulang`, desc: 'Kartu SRS jatuh tempo hari ini', mode: 'ulasan' };
    if (knownN === 0)
      return { icon: '🃏', label: 'Mulai dari Kartu', desc: 'Pelajari istilah dasar dulu', mode: 'kartu' };
    if (pct < 20)
      return { icon: '🃏', label: 'Lanjutkan Kartu', desc: `${knownN} sudah hafal, terus!`, mode: 'kartu' };
    if (pct < 50)
      return { icon: '❓', label: 'Coba Kuis', desc: 'Uji pemahaman kamu', mode: 'kuis' };
    if (pct < 70)
      return { icon: '📋', label: 'Soal JAC Official', desc: 'Latihan soal ujian asli', mode: 'jac' };
    return    { icon: '🎯', label: 'Simulasi Ujian', desc: 'Kamu sudah siap!', mode: 'simulasi' };
  };
  const suggestion = getSuggestion();

  return (
    <div style={{ padding: '0 16px 24px', maxWidth: T.maxW, margin: '0 auto', animation: 'fadeIn 0.3s ease' }}>
      {/* Header */}
      <div style={{ padding: '16px 0 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button onClick={onChangeTrack} style={{
          fontFamily: 'inherit', fontSize: 11, padding: '5px 10px',
          borderRadius: T.r.pill, cursor: 'pointer',
          background: trackInfo.bg, border: `1px solid ${trackInfo.color}33`,
          color: trackInfo.color, display: 'flex', alignItems: 'center', gap: 4,
        }}>
      ...
      {dueCount > 0 && (
        <button
          ...
          style={{
            ...
            fontFamily: 'inherit', borderRadius: T.r.lg, cursor: 'pointer',
            background: 'linear-gradient(135deg, rgba(251,191,36,0.15), rgba(245,158,11,0.08))',
            border: `1.5px solid ${T.borderActive}`, color: T.text, textAlign: 'left',
      ...
      {/* Progress Card */}
      <div style={{
        padding: '20px 16px', borderRadius: T.r.xl,
        background: T.accentSoft, border: `1px solid ${T.border}`,
        marginBottom: 12, position: 'relative', overflow: 'hidden',
      }}>
        <div style={{ position: 'absolute', top: -30, right: -30, width: 100, height: 100, borderRadius: '50%', background: `radial-gradient(circle, ${T.amber}12, transparent 70%)` }} />
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, position: 'relative' }}>
          <div style={{
            ...
            <div style={{ width: 50, height: 50, borderRadius: '50%', background: T.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 800, color: T.gold }}>
              {pct}%
            </div>
          </div>
          <div style={{ flex: 1 }}>
        ...
        {srsStats && srsStats.total > 0 && (
          <div style={{ marginTop: 12, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {[
              ...
              <span key={s.label} style={{ fontSize: 10, padding: '2px 6px', borderRadius: T.r.pill, background: `${s.color}12`, color: s.color, border: `1px solid ${s.color}25` }}>
                {s.n} {s.label}
              </span>
            ))}
          </div>
      ...
      <button
        ...
        style={{
          ...
          color: T.textBright, textAlign: 'left',
          boxShadow: T.shadow.glow, transition: 'transform 0.15s',
      ...
      {/* Stats Row */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 12, animation: 'slideUp 0.4s ease 0.15s both' }}>
        {[
          { n: CARDS.length,         label: 'Kartu',      icon: '🃏' },
          { n: JAC_OFFICIAL.length,  label: 'Soal JAC',   icon: '📋' },
          { n: wgTotal,              label: 'Soal Teknis', icon: '🎓' },
        ].map((s, i) => (
          <div key={i} style={{ padding: '12px 8px', borderRadius: T.r.md, background: T.surface, border: `1px solid ${T.border}`, textAlign: 'center' }}>
            <div style={{ fontSize: 16, marginBottom: 2 }}>{s.icon}</div>
            <div style={{ fontSize: 16, fontWeight: 800, color: T.gold }}>{s.n.toLocaleString()}</div>
      ...
      {/* Quick Actions */}
      <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, color: T.textDim, textTransform: 'uppercase', marginBottom: 8, animation: 'slideUp 0.4s ease 0.2s both' }}>
        Akses Cepat
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, animation: 'slideUp 0.4s ease 0.2s both' }}>
        {[
          ...
          <button key={m.key} onClick={() => onNavigate(m.key)} style={{
            fontFamily: 'inherit', padding: '14px 12px', borderRadius: T.r.md, cursor: 'pointer',
            background: T.surface, border: `1px solid ${m.key === 'ulasan' && dueCount > 0 ? T.borderLight : T.border}`,
      ...
      <div style={{ textAlign: 'center', marginTop: 24, fontSize: 10, color: T.textFaint, animation: 'fadeIn 0.5s ease 0.3s both' }}>
