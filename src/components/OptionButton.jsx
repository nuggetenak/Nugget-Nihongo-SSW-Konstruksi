...
export default function OptionButton({ idx, text, selected, isCorrect, onSelect, subText }) {
  ...
  return (
    <button
      ...
      disabled={answered}
      style={{
        width: "100%", padding: "13px 14px", fontSize: 13, lineHeight: 1.55,
        fontFamily: T.fontJP, borderRadius: T.r.md,
        background: getBg(), border: `1.5px solid ${getBorder()}`,
    ...
    >
      <span style={{
        ...
        background: badge.bg, border: `1.5px solid ${getBorder()}`,
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: 11, fontWeight: 800, marginTop: 1, color: badge.color,
        transition: "all 0.2s",
      }}>{badge.text}</span>
      <span style={{ flex: 1 }}>
        {text}
        {subText && <span style={{ display: "block", fontSize: 11, color: T.textMuted, marginTop: 2, fontFamily: T.font }}>{subText}</span>}
      </span>
    </button>
  );
}
