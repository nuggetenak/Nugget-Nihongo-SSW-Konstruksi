// ─── tests/kenchiku-data.test.js ─────────────────────────────────────────────
// Phase B: Schema validation for KENCHIKU_SETS data. (REF-9: now in quiz-sets.js)
// ─────────────────────────────────────────────────────────────────────────────
import { describe, it, expect } from 'vitest';
import { getQuizSetsForTrack } from '../data/quiz-sets.js';
const KENCHIKU_SETS = getQuizSetsForTrack('kenchiku').filter(s => s.track === 'kenchiku');

describe('Phase B — KENCHIKU_SETS data', () => {
  it('exports an array of sets', () => {
    expect(Array.isArray(KENCHIKU_SETS)).toBe(true);
    expect(KENCHIKU_SETS.length).toBeGreaterThanOrEqual(3);
  });

  it('each set has required fields (id, title, subtitle, emoji, questions)', () => {
    for (const set of KENCHIKU_SETS) {
  ...
  it('total question count ≥ 45', () => {
    const total = KENCHIKU_SETS.reduce((n, s) => n + s.questions.length, 0);
  ...
  it('set IDs are unique and prefixed correctly', () => {
    const ids = KENCHIKU_SETS.map((s) => s.id);
    expect(new Set(ids).size).toBe(ids.length);
    expect(ids.every((id) => id.startsWith('kenchiku-'))).toBe(true);
  });
  ...
  it('SIPIL and KENCHIKU set IDs do not overlap', () => {
